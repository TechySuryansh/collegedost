...
const PillSection = ({ title, items, viewAllLink = "#", color = "border-gray-200" }) => {
  return (
    <section className="mb-20">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-2xl font-bold font-heading text-gray-900 pl-3 border-l-4 border-brand-orange">{title}</h2>
        <Link to={viewAllLink} className="flex items-center gap-2 text-sm font-bold text-brand-indigo hover:text-brand-orange transition-colors group">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {items.map((item, index) => {
          // Support both 'link' and 'href' properties for flexibility
