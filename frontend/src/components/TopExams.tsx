...
const TopExams: React.FC = () => {
    ...
    return (
        <section className="py-10 mb-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-6 font-primary">Top Exams this month</h2>
            <div className="flex flex-wrap gap-3">
                {exams.map((exam) => (
                    <Link
                        key={exam.examSlug}
                        href={`/exams/${exam.examSlug}`}
                        className="px-6 py-2.5 rounded-full border border-purple-400 text-purple-700 hover:bg-purple-50 transition-colors text-sm font-medium whitespace-nowrap"
                    >
                        {exam.examName}
                    </Link>
                ))}
