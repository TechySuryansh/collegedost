...
import { FaBalanceScale, FaUniversity, FaSearch, FaTimes, FaPlus, FaTrophy, FaBuilding, FaMoneyBillWave, FaBriefcase, FaGraduationCap, FaMapMarkerAlt, FaCheckCircle, FaStar, FaInfoCircle } from 'react-icons/fa';
...
const CompareColleges = () => {
    ...
    const [activeSearchIndex, setActiveSearchIndex] = useState(null); // Which slot is searching

    const MAX_COLLEGES = 4;

    useEffect(() => {
        // Hydrate from URL if provided (e.g. ?id=1,2,3)
        // Implementation for future: Load from URL params
    ...
    useEffect(() => {
        setLimitReached(selectedColleges.length >= MAX_COLLEGES);
    }, [selectedColleges]);

    // Fetch full comparison data for list of IDs
    const fetchComparisonData = async (ids) => {
        try {
            ...
            if (response.data.success) {
                 ...
                 const newColleges = response.data.data.filter(c => !selectedColleges.find(existing => existing._id === c._id));
    ...
    const addCollege = async (collegeData) => {
        ...
        // Fetch detailed data for this single college to ensure we have all fields
        // Using the compare endpoint for consistency or just getBySlug
        try {
            setLoading(true);
            // We can pass the slug or ID. Search returns slug usually.
    ...
    const RenderValue = ({ value, type, currency = "INR" }) => {
        if (!value) return <span className="text-gray-400 text-sm">-</span>;
        
        if (type === 'currency') {
             if (typeof value === 'number') {
                return new Intl.NumberFormat('en-IN', { style: 'currency', currency, maximumSignificantDigits: 3 }).format(value);
             }
            ...
            <div className="container mx-auto px-4">
                <div className="text-center max-w-3xl mx-auto mb-10">
                    ...
                    <h1 className="text-3xl md:text-5xl font-bold text-gray-900 mb-4 font-heading">
                        Head-to-Head <span className="text-gradient-purple">Comparison</span>
                    </h1>
                    ...
                    {selectedColleges.map((college) => (
                        ...
                        >
                            ...
                            <div className="h-32 flex items-center justify-center mb-4 bg-gray-50 rounded-lg p-2">
                                {/* Placeholder for Logo if not exists */}
                                {college.logo ? (
                            ...
                            <table className="w-full min-w-[800px]">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-100">
                                        ...
                                        {/* Empty headers for remaining slots to maintain layout if needed */}
                                ...
                                <tbody className="divide-y divide-gray-100">
                                        ...
                                        {selectedColleges.map(c => (
                                            <td key={c._id} className="p-4 text-sm font-bold text-gray-900">
                                                {c.detailedFees?.length > 0 ? (
                                                    ...
                                                    // Fallback to average generic fee if available
                                    ...
                                    {/* Placements - Mock data logic if not in schema, checking generic schema */}
                                    <tr>
